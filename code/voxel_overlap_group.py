...
class VoxelOverlapGroup:
    """
    Aggregate per-subject Dice tables from voxel_overlap.py (each communicate
    contrast vs. pointlight interact-noninteract) and from
    ...
    *itself* across independent halves of the data, which upper-bounds how
    ...
    By default, aggregates every subject with per-subject output on disk. Pass
    explicit subject numbers (and --out_tag) to restrict to a subset (e.g. the
    independent_subs in the makefile) without overwriting the full-sample
    group outputs.
    """
    ...
    def __init__(self, args):
        ...
        self.communicate_pointlight_path = os.path.join(
            self.dataset_path, 'derivatives', 'VoxelOverlap')
        self.splithalf_path = os.path.join(
            self.dataset_path, 'derivatives', 'VoxelOverlapPointlightSplitHalf')
        self.subjs_filter = ({str(i).zfill(2) for i in args.sub_nums}
                              if args.sub_nums else None)
        communicate_out_dir = f'VoxelOverlap_{args.out_tag}' if args.out_tag else 'VoxelOverlap'
        splithalf_out_dir = (f'VoxelOverlapPointlightSplitHalf_{args.out_tag}' if args.out_tag
                              else 'VoxelOverlapPointlightSplitHalf')
        self.communicate_pointlight_out_path = os.path.join(self.dataset_path, 'derivatives', communicate_out_dir)
        self.splithalf_out_path = os.path.join(self.dataset_path, 'derivatives', splithalf_out_dir)
        Path(self.communicate_pointlight_out_path).mkdir(parents=True, exist_ok=True)
        Path(self.splithalf_out_path).mkdir(parents=True, exist_ok=True)

    def _load_tables(self, path, script_name):
        ...
        if self.subjs_filter is not None:
            df = df[df['subject'].astype(str).str.zfill(2).isin(self.subjs_filter)]
        print(f'Found {df["subject"].nunique()} matching subjects\' Dice tables in {path}')
        return df

    def load_subject_tables(self):
        communicate_df = self._load_tables(self.communicate_pointlight_path, 'voxel_overlap.py')
        splithalf_df = self._load_tables(self.splithalf_path, 'voxel_overlap_pointlight_splithalf.py')
        return communicate_df, splithalf_df

    def save_group_tables(self, communicate_df, splithalf_df):
        communicate_outfile = os.path.join(self.communicate_pointlight_out_path, 'group_dice_coefficients.csv')
        communicate_df.to_csv(communicate_outfile, index=False)
        print(f'Saved {communicate_outfile}')

        splithalf_outfile = os.path.join(self.splithalf_out_path, 'group_dice_coefficients.csv')
    ...
    def plot_splithalf_summary(self, splithalf_df):
        ...
        fig.tight_layout()
        outfile = os.path.join(self.splithalf_out_path, 'dice_group_summary.png')
    ...
    def plot_comparison_summary(self, communicate_df, splithalf_df):
        ...
        for ax, hemi in zip(axes, hemispheres):
            ...
            ceiling_percents[0] -= 4
            ceiling_percents[-1] += 3
        ...
        fig.tight_layout()
        outfile = os.path.join(self.communicate_pointlight_out_path, 'dice_group_summary.png')
        fig.savefig(outfile, dpi=300, bbox_inches='tight')
        plt.close(fig)
        print(f'Saved {outfile}')
...
def main():
    ...
    parser.add_argument('sub_nums', nargs='*', type=int,
                         help='Subject numbers to include (default: every subject with per-subject '
                              'output on disk).')
    parser.add_argument('--dataset_path', '-d', type=str,
                         default='/orcd/data/ngk/001/users/emaliem/sts_communication')
    parser.add_argument('--out_tag', type=str, default='',
                         help='If set, write to derivatives/VoxelOverlap_<out_tag>/ and '
                              'derivatives/VoxelOverlapPointlightSplitHalf_<out_tag>/ instead of the '
                              'unsuffixed directories, so a run on a subject subset does not overwrite '
                              'the full-sample group outputs.')
