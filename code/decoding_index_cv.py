...
class DecodingIndexCV:
    """
    Cross-validated version of the Haxby-style within/between decoding
    index (see decoding_index.py): each pairwise pattern correlation is
    computed between condition betas estimated from *disjoint* sets of runs
    (fold A = runs 1-5, fold B = runs 6-9; NilearnGLMRunwise per-run betas)
    instead of from a single all-runs-pooled GLM.
    ...
    collinear the GLM design is -- unlike the pooled decoding index, where
    imperfect regressor orthogonality (drift/motion interacting with block
    timing) correlates the *estimation noise* of different conditions'
    betas and inflates same-fold pattern correlations even absent any true
    signal (see the whole-brain background-bias investigation in
    methods.md). Any true pattern similarity, in contrast, is stable across
    independent runs and survives the split.
    ...
    def run(self):
        ...
        out_file = os.path.join(self.out_path, f'sub-{self.subject}_decoding_index_cv.csv')
